...
- `PlanSession.start()`から`close()`または`cancel()`までKernel、FRAME、RATE、buffer、collector、Extension trigger状態を保持
...
PortablePlanIR schema 0.2はNode parameter、同期tolerance/missing policy、adapter buffer、callable gap policyを追加する。readerは0.1のoptional field欠落を受理し、FRAME/RATE parameterをtime descriptorから復元する。`bind_plan()`はSource、Kernel、collector、Extension slotとConfig scopeの完全一致を検証してからGraphを再構築する。
