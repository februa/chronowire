...
現在の最小mixed実装は、native prefixから末尾の単一Python islandへ進む一方向境界である。
`StageDescriptor.input_port_ids/output_port_ids`から境界を特定し、native prefixの所有結果を
adapterへ一回copyしてPython Stageを一回dispatchする。Pythonからnativeへ戻る
`resume(output_batches)`、複数island、複数入力、zero-copyは次の継続状態実装で扱う。
