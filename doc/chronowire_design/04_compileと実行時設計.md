...
`0.2.0.dev0`の最初の実装段階ではfinite Sourceの`flush()`と`close()`を提供する。RealtimeSourceの停止、fail、ingress drain policyはSource lifecycle実装時に追加し、非finite Sourceを暗黙にEOF扱いしない。
