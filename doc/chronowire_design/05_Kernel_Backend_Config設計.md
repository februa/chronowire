...
> **正式名称:** 利用者の処理意味はOperation、compile済み不変実装factoryはKernel、
> run-localな実行主体はKernelStateとする。v0.4のcompile前`Kernel` protocolは内部
> `KernelProvider`へ移し、通常のFlow利用者へABIやKernelStateを露出しない。
...
以下のprotocolは新設計の内部`Kernel`/`KernelState`へ責務を引き継ぐ。
...
Python callableは実行時の特別分岐で直接呼ばず、`KernelState`を生成する通常のKernel lifecycleへ正規化する。Python callableはstateless契約とし、run間で継続する可変状態が必要な処理は明示的なstateful Kernelとして実装する。

## 1.3 stateful Kernel

`Kernel`はcompile済み係数、実行方式、workspace要件など、複数Session間で共有可能な不変factoryである。
可変状態、native handle、実workspaceは`create_state()`が生成する`KernelState`だけに保持する。
KernelStateは受動的データ構造ではなく、`process()`と必要に応じた`flush()`、`close()`を担う
能動的な実行主体である。

v0.4のPython runtimeは`process()`とcapability検出した`close()`を実行し、Plan全体の
`ContinuousSession.flush()`ではSource、FRAME、bufferをdrainする。KernelState固有の
output-producing `flush()`は、0件・複数件Emissionの返却規則をportable ABIへ定義してから有効化する。
それまでは`supports_flush=True`をPython Kernelへ暗黙適用せず、未対応Executorが明示的に拒否する。

必要な契約:

- Sessionごとに新しいKernelStateを生成
- thread safety
- snapshot可否
- run間の状態継続可否

`Session`は一回だけ実行し、別SessionのKernelStateを構造的に参照できない。`ContinuousSession`は
`start()`時に一度だけKernelStateを生成し、複数の`run_until()`間で同じinstanceを保持する。
`close()`、`cancel()`、または実行失敗後にそのinstanceを再利用せず、同じPlanから作る次のSessionは
必ず新しいKernelStateを得る。
...
BackendはNodeのアルゴリズム実装をcompileする。Plan全体のready判定、rate cursor、frame history、buffer寿命はExecutorの責務であり、Backendへ含めない。Native KernelをPython Executorから呼ぶ構成と、NativeExecutorから呼ぶ構成を区別する。詳細は[10_Native_Executor設計方針.md](10_Native_Executor設計方針.md)を参照する。

v0.3ではBackendが返す`Kernel`が任意で`NativeKernel`を実装できる。
...
v0.4で固定shape batchを処理するfactoryは`NativeBatchKernel`、sessionは
`NativeBatchKernelState`を実装する。Executorは一つのread-only f64 memoryview、item count、
...
immutable bytesとして`Plan.create_session()`時にCppExecutorへbindする。pointer、Python
...
Configは実行中データを運ばない。時刻とともに変化する値はFlow、最新値参照はStateFlow、Operation内部の継続状態はKernelState、診断はDiagnosticとして扱う。
...
Configは不変なのでrun中の競合書込みを持たない。KernelStateはContinuousSessionが所有し、Scheduler管理下でのみ更新する。並列Stageで共有状態が必要な場合は、StateFlowによる順序と更新境界を明示する。
