...
`Session.flush()`ではSource、FRAME、bufferをdrainする。KernelState固有の
...
`Session.run()`は呼出しごとにKernelStateを生成・破棄し、再実行へ状態を持ち越さない。
同じSessionで`start()`を選んだ場合は開始時に一度だけKernelStateを生成し、複数の`run_until()`間で
同じinstanceを保持する。`close()`、`cancel()`、または実行失敗後にそのKernelStateを再利用しない。
...
Configは不変なのでrun中の競合書込みを持たない。KernelStateはSessionが所有し、Scheduler管理下でのみ更新する。並列Stageで共有状態が必要な場合は、StateFlowによる順序と更新境界を明示する。
