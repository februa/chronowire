...
指定されたFlowを観測終端とし、それらへ至る祖先Nodeの和集合だけをPlanへ含める。
...
Flow利用者へKernel、ABI、Sessionを露出する公開設計にはしない。
...
ChronowireはPython APIを入口とするが、GraphとPlanの中心表現は言語非依存に近づける。
