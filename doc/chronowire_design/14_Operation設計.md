...
| `Session` | Plan全体のrun-localな可変状態 | `Session`、`ContinuousSession` |
...
mixed Cpp ContinuousSessionは次段階であり、未対応時はNode/Port/Stage/bindingを含む明示エラーにする。
...
- `session`: `ContinuousSession.start()`からclose/cancelまで履歴とworkspaceを保持
...
| v0.4 `CompiledKernel` | `Kernel`。compile済み不変factory |
| v0.4 `CompiledKernelSession` | `KernelState`。run-local実行主体 |
...
Pythonの旧公開class名は一リリースだけdeprecated aliasとして同じ実装を参照する。内部実装は正式名称
だけを使い、PortablePlanIR schemaとC ABI IDはclass名を保存しないため変更しない。

互換性は次の境界ごとに扱う。

| 境界 | v0.4方針 |
|---|---|
| 公開Python名 | `ExecutionPlan`、`ExecutionSession`、`PlanSession`、`PlanSessionState`、`CompiledKernel`系、`BoundExecutionPlan`、`PlanSessionError`は正式型へのdeprecated aliasとしてv0.5まで残す |
| 内部Python名 | 二重実装を避け、`Plan`、`Session`、`ContinuousSession`、`Kernel`、`KernelState`へ即時統一する |
| method | `create_plan_session()`だけ警告付きで残し、新規コードは`create_continuous_session()`を使う |
...
| import path | `chronowire`トップレベルの旧名だけを移行入口にする。内部module pathの互換性は保証しない |
...
- v0.4互換alias `cw.CompiledKernel`、`cw.CompiledKernelSession`、`cw.ExecutionPlan`、
  `cw.ExecutionSession`、`cw.PlanSession`のv1.0前廃止
