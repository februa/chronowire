...
native_plan = cw.compile(outputs, backend="cpp")
```

`backend="python"`はPython Implementationを選ぶ。`backend="cpp"`は同じoperation IDのC++
Implementationを選び、decoratorに付属するPython計算本体を実行しない。ただしOperationSpecは両Backend
に共通する意味論の正本として使う。
...
| Backend | Executor | 用途 |
|---|---|---|
| Python | PythonExecutor | 意味論の基準、試作、デバッグ |
...
plan = cw.compile([cw.output(spectrum)], backend="cpp")
```
...
cpp_plan = cw.compile([cw.output(power_flow)], backend="cpp")
```
...
Python参照実装が存在しても`backend="cpp"`から暗黙に呼ばない。
...
- CppExecutorのidentity/固定CBF resolver tableはnative manifest bindingへ段階的に置換する
