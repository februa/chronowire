...
2026-07-22時点の初期実装は、all-Python one-shot Planを1つの最大islandとして
C++所有のyield/resume状態から実行する。0/1/複数Emission、fan-out、status/Diagnostic、
例外後の再実行をPythonExecutorと照合済みである。mixed Planの実batch受け渡し、
fixed-schema zero-copy、複数Stage、Cpp PlanSessionは次段階であり、未対応時は
