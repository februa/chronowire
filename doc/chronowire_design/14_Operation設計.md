...
Chronowire v0.4までの`Kernel`、`CompiledKernel`、`CompiledKernelSession`、
`NativeValueSchemaProvider`は実装上の先行概念であり、公開設計としては本書の
`OperationSpec`、`ImplementationSpec`、内部`CompiledOperation`、`OperationSession`へ移行する。
...
| `Session` | run-localな可変状態 | `OperationSession` |
...
`CompiledOperation`、実行中の履歴、workspace、native handleは`OperationSession`が所有する。
...
最新状態はStateFlow、Operation内部履歴はOperationSessionへ置く。
...
`inputs` Edgeである。FIR履歴、積分器、FFT workspace、native handleはOperationSessionである。
...
- OperationSessionのcreate/process/flush/destroy
...
2026-07-22時点では、all-Python one-shot Planと、native prefixから末尾の単一Python islandへ
進むmixed Planを実装済みである。StageDescriptorは外部入力・出力Port IDを保持し、mixed境界は
C++所有結果からadapterへ一回copyしてStage単位でdispatchする。0/1/複数Emission、fan-out、
status/Diagnostic、例外後の再実行をPythonExecutorと照合済みである。Pythonからnativeへ戻る境界、
fixed-schema zero-copy、複数island、複数入力、Cpp PlanSessionは次段階であり、未対応時は
Node/Port/Stage/bindingを含む明示エラーにする。
...
- Scheduler、buffer、論理時間、status/Diagnostic配送などExecutionPlanの実行意味論
...
- `stateless`: OperationSessionは軽量でもよく、run間で状態を共有しない
- `session`: `PlanSession.start()`からclose/cancelまで履歴とworkspaceを保持
- gap=`reset`: 欠落後の最初の入力前に新しい状態へreset
- gap=`accept`: OperationSpecが欠落を受理しDiagnosticを出力へ伝播するときだけ許可

ExecutionPlanやImplementationBindingへrun-local可変状態を置かない。例外後のOperationSessionを再利用せず、
...
| v0.4実装 | Operation設計 |
|---|---|
| `Kernel` | `OperationSpec`を持つOperation declaration |
| `CompiledKernel` | 内部`CompiledOperation` |
| `CompiledKernelSession` | `OperationSession` |
...
これらは一度にrenameしない。PortablePlanIR reader、既存Plan、testsを壊さない段階的migrationを行う。
...
12. **末尾Python islandまで初期実装済み**: CppRuntimeMetricsでGIL解放契約、native Stage dispatch、
   Python境界callback、公開Emission復元、batch変換を分離計数。native prefixからplain callableへ
   一回batch dispatchできる。Pythonからnativeへ戻る境界とzero-copyは未実装
...
共分散積分はOperationSession、最新重みはLATEST Edge、固定steeringとdiagonal loadingはConfigに置く。
...
- `cw.Kernel`、`cw.CompiledKernel`、`cw.CompiledKernelSession`のトップレベル公開廃止
