...
Operation対応schemaでは`KernelAbiDescriptor`を直接の意味論正本にせず、OperationDescriptorから選択済み
Implementation/ABI descriptorを参照する。schema 0.3 readerは維持し、identity/固定CBF等の既存ABIを
同じoperation IDのlegacy implementationとして復元する。新旧schemaの具体version番号はOperation model
実装時に決め、v0.4 Planを黙って別Operationへbindしない。
