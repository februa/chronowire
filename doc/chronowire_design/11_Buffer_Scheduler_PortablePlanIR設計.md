...
v0.1の`TimeDescriptor`は`exact`、`finite`、任意の`generation_end`を持つ。有限性を証明できても終了時刻を一般Iterableから静的に求められない場合、`generation_end`は推測せずnullとする。`EdgeDescriptor`はrequired flagと任意のadapter Buffer IDを持ち、v0.1の公開Flow入力はrequired、変換不要時のadapterはnullとする。
