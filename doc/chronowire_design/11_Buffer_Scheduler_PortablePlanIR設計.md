...
`GapMarker`自体は利用者値の`Emission`ではない。Scheduler、FRAME、RATE、merge、stateful KernelStateが
欠落境界を認識するためのruntime control recordであり、PortablePlanIRにはgap処理policyを記録する。
...
v0.1 Python Executorのstateful Kernel既定`RESET`は、gap直後の入力を実行する前に同じ`Kernel`から新しいrun-local sessionを生成することで実現する。FRAMEは同時に未完成履歴とhop skip状態を破棄する。
...
MAP Nodeは`OperationDescriptor`を参照する。公開Operation、Implementation、shape、Configとの完全な対応は
[14_Operation設計.md](14_Operation設計.md)を正本とする。

### 7.1.1 OperationDescriptor

- operation ID
- input名とinput/output Port ID
- resolved input/output ValueSchemaDescriptor
- Config scope ID、leaf dependency、digest
- time、Emission件数、status、state、gap規則
- 選択implementation ID、ABI version、execution domain
- Implementation binding slot、workspace descriptor

OperationDescriptorへPython function、shape resolver、pointer、module handle、library pathを保存しない。
`ImplementationSpec`のportableな選択結果だけを記録し、process-local実体はExecutionBindingsへ置く。
...
`SourceDescriptor`は`PULL_CONTROLLED`または`REALTIME_PUSH`、request単位、有限性、burst上限、ingress Buffer ID、overflow policy、gap policyを持つ。`BindingDescriptor`はSource、Operation implementation、plain Python callback、collector、Extensionのprocess-local実体へ結び付ける安定slot IDとABI/schema versionを持つ。schema 0.3のKernel slotは読込み時にlegacy Operation bindingへ正規化する。
...
v0.3/v0.4の固定shape native Kernelは`NativeValueSchemaProvider`で入力Port shapeから出力Port shapeを
compile時に解決する。Operation移行ではshape規則をOperationSpecへ移し、Backend選択前にunifyする。
shape resolver callableはIRへ入れずresolved schemaだけを保存する。v0.3固定CBFでは`frame_size × channels`から`beams × frame_size`を生成し、
...
Operation対応schema 0.4では`KernelAbiDescriptor`を直接の意味論正本にせず、`OperationDescriptor`から
選択済み`ImplementationDescriptor`を参照する。OperationDescriptorはNode/Port、入力名、resolved schema、
Config scope/path/digest/leaf型、time/Emission/status/state/gap規則、implementation ID/ABI、binding slotを
保持する。ImplementationDescriptorはBackend、process model、workspace、CPU feature、native互換性を
保持する。Python callable、shape resolver、native pointer、module handle、library pathはどちらにも保存しない。
