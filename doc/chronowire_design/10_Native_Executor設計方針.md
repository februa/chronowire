...
`0.4.0.dev0`では、同じ固定shape PlanをC++ Executorでも実行する五つ目の構成を追加する。Cython
...
`0.4.0.dev0`の最小実装では、PortablePlanIRの`SOURCE → RATE → FRAME → MAP` descriptorと
process-local Source/Kernel bindingからrun-local C++ sessionを生成する。sessionはprepacked f64値、
signed i64 source tick、status、固定CBF係数を所有し、RATE、FRAME、CBF、NoCollect/Latest/Boundedの
保持選択をGILなしで実行する。NoCollectでは出力値をPython境界へcopyしない。Bounded/Latestでは
保持対象だけを公開Emissionへ復元し、DiagnosticはC++が伝播したsource provenance indexから戻す。

この段階のKernel dispatchはversion付き固定CBF ABIをC++ runtimeが直接認識する方式であり、汎用
function pointer tableや複数native Kernel chainはまだ実装しない。`PlanSession`、Extension、Python
callback、gap reset、INVALID partition、metadata table、fan-outも暗黙fallbackせず明示エラーとする。
意味論と再測定結果は[13_CppExecutor移行測定.md](13_CppExecutor移行測定.md)を正本とする。
