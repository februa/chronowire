...
`Plan.run()`と`create_session()`は`Executor`を選択でき、既存runtimeを`PythonExecutor`として使用する。これはExecutor差し替え境界の固定であり、まだNative実行を意味しない。
...
Python値、任意callback、Extension、EOF padding、Sessionの段階実行modeは未対応である。
...
有限`Session`は排他的論理時間ごとの決定的snapshot、flush、close、cancelを提供する。one-shot
