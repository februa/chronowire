...
Kernel dispatchはversion付きABI resolver tableでidentity f64と固定CBFを選択する。fan-outでは親batchを
...
単一有限f64 vector Source、`FRAME(pad_end=False)`、statelessな登録済みABI、既定RuntimeOptionsが
v0.4のportable範囲である。realtime push、複数Source/merge、mutable native workspace、incremental
