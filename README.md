...
このconformanceは次の四構成で、値、interval、sequence、status、Diagnosticを比較する。
...
最後の構成では`f64_vector_source()`が固定channel shapeを宣言し、Cython Executorが
RATE/FRAMEをbatch化してCython CBFへ一回で渡す。EmissionごとのPython dispatchはなく、
