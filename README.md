...
- required Node抽出、interval不一致warning、単一thread決定的runtime
- `NoCollect`、`Latest`、`Bounded`、`Sink`
- collectorと独立したExtension、劣化結果を保存する`Snapshot`
- Logical Graph/ExecutionPlanのJSON・DOT export

この一覧は現在動作するbaselineであり、v0.1 release完了を意味しない。v0.1ではさらに、共有`PortBuffer`、demand-driven Scheduler、完全なPortablePlanIR、pull/realtime Source分類、bounded realtime ingressとgap伝播まで実装する。
