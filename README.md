...
- 読み取り専用の共有`PortBuffer`、consumer cursor、構造解析による静的capacity
- Node/Port/Edge/Buffer/Time/Source/Bindingを持つPortablePlanIR round-trip
- `NoCollect`、`Latest`、`Bounded`、`Sink`
- collectorと独立したExtension、劣化結果を保存する`Snapshot`
- Logical Graph/ExecutionPlanのJSON・DOT export

この一覧は現在動作するbaselineであり、v0.1 release完了を意味しない。v0.1ではさらに、Portごとのfrontier/capacity証明を含む完全なPortablePlanIR、pull/realtime Source分類、bounded realtime ingressとgap伝播まで実装する。
